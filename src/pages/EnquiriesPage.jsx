...
function QuoteField({ label, value, onChange, onGenerate, trips = [], customerPhone = '' }) {
  ...
  // Build WhatsApp URL — strip non-digits from phone, prepend 91 if needed
  const waPhone = customerPhone.replace(/\D/g, '').replace(/^0+/, '')
  const waPhoneFormatted = waPhone.length === 10 ? '91' + waPhone : waPhone
  const waUrl = waPhoneFormatted && value
    ? `https://wa.me/${waPhoneFormatted}?text=${encodeURIComponent(value)}`
    : null
        ...
        <div className="flex items-center gap-2">
          ...
          {waUrl ? (
            <a
              ...
              title={`Send to ${customerPhone} on WhatsApp`}
              className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#25D366] hover:bg-[#1ebe5d] transition-colors"
            >
            ...
            <span
              title={!value ? 'Generate a quote first' : 'Enter customer phone to send via WhatsApp'}
              className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-200 cursor-not-allowed"
            >
        ...
        <form onSubmit={validateAndSubmitEnquiry} className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <QuoteField
              ...
              customerPhone={eForm.customerPhone}
            />
